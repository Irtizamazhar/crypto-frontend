...
import { fetchCoin, fetchNews, fetchKlines, fetchMarket } from "../services/api";
...
export default function AIChatBot() {
  const { user } = useAuth();             // auth state (do NOT early-return)
  const isAuthed = !!user;
  ...
  const [userName, setUserName] = useState(
    typeof window !== "undefined" ? localStorage.getItem("chatbot_user_name") || "" : ""
  );
  const messagesEndRef = useRef(null);

  const { addAlert, limits } = useAlerts();

  // Close if user signs out while open
  useEffect(() => {
  ...
  useEffect(() => {
    if (userName) localStorage.setItem("chatbot_user_name", userName);
  }, [userName]);
  ...
  function formatPrediction(analysis, symbol) {
    ...
    const { trend, currentPrice, volatility } = analysis;
  ...
  async function askAI(question) {
    ...
    const userLabel = userName || "there";
    ...
    try {
      if (q.includes("how are you")) {
        return { text: `I'm great, ${userLabel}! Systems green and data live. How can I help?`, meta: stamp };
      }
      ...
      if (q.includes("thanks") || q.includes("thank")) {
        return { text: `You’re welcome, ${userLabel}! Want a plan for BTC or your favorite coin?`, meta: stamp };
      }
  ...
  return (
    <>
      {/* Only render the UI when authenticated */}
      {isAuthed && (
        <>
          {/* FAB */}
          <motion.button
      ...
      {/* Styles (safe to keep always rendered) */}
      <style jsx>{`
