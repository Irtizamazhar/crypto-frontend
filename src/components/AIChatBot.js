...
import { Bot, X, Send, User, Loader2, TrendingUp, TrendingDown, Star, Calendar, BarChart3, Lightbulb, Zap, AlertTriangle, Rocket } from "lucide-react";
...
export default function AIChatBot() {
  ...
  const [messages, setMessages] = useState([
    { 
      from: "bot", 
      text: "Hello! I'm your advanced crypto trading assistant with predictive analytics. I can help with real-time market data, AI-powered predictions, sentiment analysis, and trading strategies. How can I assist you today? 🚀",
      timestamp: new Date(),
      type: "greeting"
    },
  ]);
  ...
  const simulateTyping = async (callback, duration = 1000) => {
    setTyping(true);
    await new Promise(resolve => setTimeout(resolve, duration + Math.random() * 500));
    setTyping(false);
    callback();
  };

  // Get trending coins based on multiple factors
  async function getTrendingCoins(limit = 5) {
    try {
      const marketData = await fetchMarket({ per_page: 100 });
      
      // Enhanced scoring: volume + price change + momentum
      const scoredCoins = marketData.map(coin => {
        ...
        return {
          ...coin,
          score: volumeScore + priceChangeScore + momentumScore
        };
      });
      
      // Sort by score and filter positive momentum
      const trending = scoredCoins
        .filter(coin => coin.changePercent24Hr > 1) // Only coins with positive momentum
  ...
  async function analyzeMarketCondition(symbol) {
    try {
      ...
      const trend = trendStrength >= 3 ? "strong-bullish" : 
  ...
  function generateRecommendation(analysis, symbol) {
    ...
    // Trading strategy based on multiple factors
    if (trend === "strong-bullish" && rsi < 70) {
      recommendation += `💚 *STRONG BUY SIGNAL* 💚\n`;
      recommendation += `Multiple indicators align for upward movement.\n`;
      recommendation += `Entry: $${currentPrice.toFixed(2)}\n`;
      recommendation += `Stop-loss: $${sma20.toFixed(2)}\n`;
      recommendation += `Target: $${(currentPrice * 1.05).toFixed(2)} (+5%)\n`;
    } 
    else if (trend === "weak-bullish") {
      recommendation += `🟡 *CAUTIOUS BUY* 🟡\n`;
      recommendation += `Moderate bullish signals. Wait for confirmation.\n`;
      recommendation += `Entry above: $${(currentPrice * 1.01).toFixed(2)}\n`;
    }
    else if (trend === "weak-bearish") {
      recommendation += `🟠 *CAUTION ADVISED* 🟠\n`;
      recommendation += `Bearish pressure building. Consider reducing exposure.\n`;
    }
    else if (trend === "strong-bearish") {
      recommendation += `🔴 *SELL/AVOID* 🔴\n`;
      recommendation += `Strong downward momentum. Wait for reversal signals.\n`;
    }
  ...
  // Get market sentiment from news and social media trends
  async function getMarketSentiment(symbol) {
    try {
      ...
      if (news.length > 0) {
        ...
        const positiveWords = ['up', 'bull', 'gain', 'surge', 'rally', 'positive', 'growth'];
        const negativeWords = ['down', 'bear', 'drop', 'crash', 'fall', 'negative', 'loss'];
  ...
  // Main AI function with enhanced capabilities
  async function askAI(question) {
    const q = question.toLowerCase();
    const user = userName || "there";
    
    try {
      // Enhanced personal conversation
      ...
      if (q.includes("my name is") && !userName) {
        ...
        if (nameMatch && nameMatch[1]) {
          ...
          setUserName(newName);
          return `Wonderful to meet you, ${newName}! 👋 I'll remember your name for our future conversations. How can I assist with your crypto journey today?`;
        }
      }

      if (q.includes("what should i call you")) {
        return `You can call me CryptoAlpha! 🤖 I'm your 24/7 trading companion, always ready with market insights and analysis.`;
      }
      ...
      if ((q.includes("predict") || q.includes("forecast") || q.includes("will") || q.includes("going to")) && !q.includes("how")) {
        ...
        const normalizedSymbol = coinSymbol === "bitcoin" ? "BTC" : 
                               coinSymbol === "ethereum" ? "ETH" : coinSymbol.toUpperCase();
      ...
      if (q.includes("sentiment") || q.includes("mood") || q.includes("feeling")) {
        ...
        if (coinSymbol === "crypto") {
          ...
          const ethSentiment = await getMarketSentiment("ETH");
          
          return `🌡️ *Overall Market Sentiment:*\n\n• Bitcoin: ${btcSentiment}/100 ${btcSentiment > 60 ? '😊' : btcSentiment < 40 ? '😟' : '😐'}\n• Ethereum: ${ethSentiment}/100 ${ethSentiment > 60 ? '😊' : ethSentiment < 40 ? '😟' : '😐'}\n\nMarket is ${(btcSentiment + ethSentiment) / 2 > 60 ? 'generally optimistic' : (btcSentiment + ethSentiment) / 2 < 40 ? 'showing concern' : 'in neutral territory'}.`;
        } else {
          const normalizedSymbol = coinSymbol === "bitcoin" ? "BTC" : 
                                 coinSymbol === "ethereum" ? "ETH" : coinSymbol.toUpperCase();
          const sentiment = await getMarketSentiment(normalizedSymbol);
          
          return `🌡️ *${normalizedSymbol} Market Sentiment:* ${sentiment}/100\n\n${sentiment > 70 ? '😊 Very Bullish' : sentiment > 60 ? '🙂 Bullish' : sentiment > 40 ? '😐 Neutral' : sentiment > 30 ? '🙁 Bearish' : '😟 Very Bearish'}\n\nBased on news and market data analysis.`;
        }
      }

      // Trending coins request with enhanced analysis
      if (q.includes("trending") || q.includes("hot") || q.includes("best coins") || q.includes("what should i trade") || q.includes("opportunit")) {
        ...
        if (trending.length === 0) {
          return "I couldn't fetch trending data right now. Please try again in a few moments.";
        }
        ...
        for (const [index, coin] of trending.entries()) {
          ...
          const trendEmoji = analysis?.trend === "strong-bullish" ? "🚀" : 
                           analysis?.trend === "weak-bullish" ? "↗️" : 
                           analysis?.trend === "weak-bearish" ? "↘️" : "🔻";
          
          response += `${emoji} ${trendEmoji} *${coin.symbol.toUpperCase()}*: $${coin.priceUsdt.toFixed(2)} (+${coin.changePercent24Hr.toFixed(2)}%)\n`;
          response += `   Volume: $${(coin.volumeUsd24Hr / 1000000).toFixed(1)}M | Trend: ${analysis?.trend.replace('-', ' ').toUpperCase()}\n\n`;
        }
      ...
      // Extract coin symbol from question with better matching
      ...
      const coinMatch = q.match(coinPattern);
      let coinSymbol = coinMatch ? coinMatch[0] : null;
      ...
      const normalizedSymbol = coinSymbol ? (coinMap[coinSymbol.toLowerCase()] || coinSymbol.toUpperCase()) : null;
      ...
      if ((q.includes("should i") || q.includes("buy") || q.includes("sell") || q.includes("trade") || 
           q.includes("analysis") || q.includes("analyze") || q.includes("prediction")) && normalizedSymbol) {
        const analysis = await analyzeMarketCondition(normalizedSymbol);
      ...
      if (q.includes("market") && (q.includes("overview") || q.includes("summary") || q.includes("how is the market"))) {
        try {
          const btcData = await fetchCoin("BTC");
          const ethData = await fetchCoin("ETH");
          const trending = await getTrendingCoins(3);
          const btcSentiment = await getMarketSentiment("BTC");
          
          let response = `🌐 *MARKET OVERVIEW - Real-time Analysis:*\n\n`;
          response += `• *BTC:* $${btcData.priceUsdt.toFixed(2)} (${btcData.changePercent24Hr >= 0 ? '+' : ''}${btcData.changePercent24Hr.toFixed(2)}%)\n`;
          response += `• *ETH:* $${ethData.priceUsdt.toFixed(2)} (${ethData.changePercent24Hr >= 0 ? '+' : ''}${ethData.changePercent24Hr.toFixed(2)}%)\n`;
          ...
          if (trending.length > 0) {
            ...
            trending.slice(0, 3).forEach(coin => {
              response += `• ${coin.symbol.toUpperCase()}: +${coin.changePercent24Hr.toFixed(2)}%\n`;
            });
          }
          
          response += `\n💡 *AI ASSESSMENT:* The market is ${btcData.changePercent24Hr >= 0 ? 'showing strength' : 'under pressure'} with ${btcSentiment > 60 ? 'positive' : 'cautious'} sentiment.`;
      ...
      // Fallback - general crypto information
      if (normalizedSymbol) {
  ...
  const formatMessage = (text) => {
    // Enhanced formatting with better styling
    return text
  ...
  return (
    <>
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        className="fixed bottom-6 right-6 z-50 p-4 bg-gradient-to-r from-purple-600 to-blue-600 rounded-full shadow-lg text-white shadow-purple-500/30"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 w-96 h-[500px] glass rounded-2xl flex flex-col overflow-hidden z-50 shadow-xl border border-slate-700/50"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-slate-900 to-slate-800">
              {messages.map((m, i) => (
                ...
                >
                  ...
                  <div className={`max-w-[75%] rounded-2xl p-3 ${m.from === "bot" 
                    ? "bg-slate-800 text-slate-200 rounded-bl-none border-l-2 border-cyan-500" 
                    : "bg-blue-600 text-white rounded-br-none border-r-2 border-blue-400"}`}
                  >
                    <div 
                      className="text-sm message-content"
                      dangerouslySetInnerHTML={{ __html: formatMessage(m.text) }}
                    />
                    <div className={`text-xs mt-1 ${m.from === "bot" ? "text-slate-400" : "text-blue-200"}`}>
