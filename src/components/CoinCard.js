...
import AlertModal from "./AlertModal"; // ← comment out if you aren't using Alerts
...
export default function CoinCard({ c }) {
  ...
  const goToTrade = () => navigate(`/trade/${c.id}`);

  const copySymbol = async (e) => {
    e.stopPropagation();
    try {
  ...
  const onOpenBinance = (e) => {
    // keep anchor default behavior but stop bubbling to card
    e.stopPropagation();
  };
  ...
  return (
    <motion.div
      ...
      onClick={goToTrade}
      onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && goToTrade()}
      whileHover={{ translateY: -4 }}
      transition={{ type: "spring", stiffness: 250, damping: 20 }}
      className="relative group rounded-2xl border border-white/10 bg-white/[0.03] p-4 hover:border-white/20 cursor-pointer focus:outline-none focus:ring-2 focus:ring-violet-500/40"
    >
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            {/* Whole card navigates, so no <Link> here */}
            <div className="font-semibold truncate hover:underline">{c.name}</div>
